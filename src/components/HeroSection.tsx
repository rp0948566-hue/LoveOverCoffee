...
const foodImages: Record<string, string> = {
  'Mojito': 'https://images.unsplash.com/photo-1513558161293-cdaf765ed2fd?q=80&w=1000&auto=format&fit=crop',
  'Pizza': 'https://images.unsplash.com/photo-1565299624946-b28f40a0ae38?q=80&w=1000&auto=format&fit=crop',
  'Cold Coffee': 'https://images.unsplash.com/photo-1517701550927-30cf4ba1dba5?q=80&w=1000&auto=format&fit=crop',
  'Sandwich': 'https://images.unsplash.com/photo-1528735602780-2552fd46c7af?q=80&w=1000&auto=format&fit=crop',
  // Fallback for any others
  'fallback': 'https://images.unsplash.com/photo-1504754524776-8f4f37790ca0?q=80&w=1000'
};
...
export default function HeroSection({ onAskMaggie, onViewMenu }: HeroSectionProps) {
  ...
  return (
    <section
      id="hero"
      ref={containerRef}
      className="relative min-h-screen flex items-center justify-center overflow-hidden animate-smooth-scale"
    >
      <div
        ref={bgRef}
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-center min-h-[60vh]">
          <div className="text-center lg:text-left order-2 lg:order-1">
            ...
            <p className="text-lg md:text-xl text-muted-foreground max-w-lg mb-10">
              Sip the magic, feel the warmth. Your cozy escape for coffee, cravings, and conversations.
            </p>
          ...
          <div className="relative order-1 lg:order-2 h-[400px] md:h-[500px]">
            <div className="absolute inset-0 flex items-center justify-center">
              {heroItems.map((item, idx) => (
                ...
                >
                  <div className="circle-container relative w-64 h-64 md:w-80 md:h-80 mb-6">
                    <div
                      className="absolute inset-0 rounded-full blur-3xl opacity-70 animate-pulse"
                      style={{ backgroundColor: item.accentColor }}
                    />
                    <div
                      className="relative w-full h-full rounded-full overflow-hidden border-4 border-white/10 shadow-2xl backdrop-blur-sm glassmorphism"
                      style={{ boxShadow: '0 0 50px rgba(176, 38, 255, 0.4)' }}
                    >
                      <img
                        key={currentIndex}
                        src={foodImages[item.name] || foodImages['fallback']}
                        alt={item.name}
                        className="absolute inset-0 w-full h-full object-cover rounded-full animate-fade-in-scale"
                        style={{
                          objectFit: 'cover'
                        }}
                      />
